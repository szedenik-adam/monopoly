...
void ws_serve_exit(ws_conn_t *conn) {
	if (conn) {
		...
		//conn->write_cb_unit.cb = 0; // Bugfix: disable new write callbacks on freed connection.
	}
}
...
//			LOG("---- STEP 4 ----");
			if (conn->frame->payload_len > 0) {
				...
				conn->frame->payload_data = new char[conn->frame->payload_len +1];
				// Add trailing zero for creating an in-place modifiable c-string.
				conn->frame->payload_data[conn->frame->payload_len] = '\0';
