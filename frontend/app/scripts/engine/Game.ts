...
module multipoly.engine {
  export class Game {
    ...
    roomService:multipoly.services.RoomService;
    ...
    onGameStateChanged(state:multipoly.models.GameState) {
      console.log(state);
      this.populateBoard();
    }
     ...
     */
    getFirstCoordinateOfFieldIndex(index:number) {
      var pos = [26, 26];
    ...
    getNextFreeFieldPosition(field) {
      var positions = this.getCoordinatesOfField(field);
      var pos = positions[0];
      var i = 0;
      /* Looking for empty position */
    ...
    getNextFreeHousePosition(field) {
      var positions = this.getCoordinatesOfField(field);
    ...
    populateBoard() {
      this.initBoard();
      var state:multipoly.models.GameState = this.roomService.getCurrentGameState();
